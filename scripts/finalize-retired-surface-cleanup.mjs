...
function patchPackageJson() {
  ...
  let nextUi = ui;
  if (nextUi && !nextUi.includes('assistant-chip-retirement:guard')) {
    nextUi = nextUi.replace('npm run notification-ui:guard &&', 'npm run notification-ui:guard && npm run assistant-chip-retirement:guard &&');
  }
  if (nextUi && !nextUi.includes('repo-rearchitecture:guard')) {
    nextUi = nextUi.replace('npm run manager-master-lightweight:guard &&', 'npm run manager-master-lightweight:guard && npm run repo-rearchitecture:guard &&');
  }
  if (nextUi) {
    nextUi = nextUi.replaceAll('assistant-chip-retirement:guard&&', 'assistant-chip-retirement:guard &&');
    nextUi = nextUi.replaceAll('repo-rearchitecture:guard&&', 'repo-rearchitecture:guard &&');
  }
  pkg.scripts['ui-source:guard'] = nextUi;
...
function writeRoadmapGuard() {
  write('scripts/repo-rearchitecture-roadmap-guard.mjs', `#!/usr/bin/env node
import { existsSync, readFileSync } from 'node:fs';
...
`);
  ...
  write('src/features/client-workspace/client-css-ownership.ts', `export type ClientCssOwner = {
  file: string;
  owns: string[];
  forbidden: string[];
};
...
`);
  ...
  write('app/client-account-popover-ratio.css', `/* Client account CSS contract. */
...
function writeRoadmap() {
  write('docs/roadmaps/repo-rearchitecture-checklist.md', `# xDisputer Repo Re-architecture Checklist
...
- [x] Fix 2 — route convention normalization
  - Files: \`proxy.ts\`, retired \`middleware.ts\` removed
...
- [x] Fix 6 — roadmap + tracker enforcement
  - Files: \`docs/roadmaps/repo-rearchitecture-checklist.md\`, \`scripts/repo-rearchitecture-roadmap-guard.mjs\`, \`package.json\`
