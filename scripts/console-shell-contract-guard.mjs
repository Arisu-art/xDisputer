...
const templateWorkspaceShell = read('components/templates/workspace/TemplateWorkspaceShell.tsx');
const managerWorkspacePage = read('app/manager-workspace/page.tsx');
const managerWorkspaceStudio = read('app/manager-workspace/studio/page.tsx');
const managerWorkspaceEngine = read('app/manager-workspace/engine/page.tsx');
...
const templateHubCss = read('app/template-workspace-hubs.css');
...
has(accountProfileRoute, 'relativeRedirect', 'Account profile API uses relative redirect');
has(accountProfileRoute, 'saveCurrentAccountProfile', 'Account profile API uses resilient save service');
has(accountProfileRoute, "revalidatePath('/', 'layout')", 'Account profile API revalidates layout after save');
notHas(accountProfileRoute, 'new URL(next, request.url)', 'Account profile API never redirects to private request URL');
...
has(templateWorkspaceShell, 'templateWorkspaceNavForPath', 'Template workspace shell owns three-hub nav contract');
has(managerWorkspacePage, 'TemplateLibraryHub', 'Manager workspace renders Template Library hub');
has(managerWorkspaceStudio, 'TemplateStudioHub', 'Manager workspace studio route renders Template Studio hub');
has(managerWorkspaceEngine, 'GenerationEngineHub', 'Manager workspace engine route renders Generation Engine hub');
...
has(layout, "import './template-workspace-hubs.css';", 'root layout imports template workspace hub CSS');
...
has(reportView, 'header={{ eyebrow, title, description }}', 'Generation report uses canonical ConsoleHeader');
...
has(packageJson, '"template-workspace:guard"', 'package defines template-workspace guard script');
...
has(templateHubCss, '.template-workspace-hub', 'Template workspace hub CSS defines hub shell');
has(templateHubCss, '@keyframes templateReadyGlow', 'Template workspace CSS defines readiness animation');
