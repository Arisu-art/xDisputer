...
import { managerTemplateActionLabel, managerTemplateAuthorityLabel } from '../lib/manager-template-ui';
...
export default function TemplateProgressiveWorkspace({ round, slots, supportingReady, managerTemplateScope = null, managedExhibits, onSelectRound, onUploadLetter, onRemoveLetter, onExhibitsChange, onMessage, onUseRoundForSourceData }: Props) {
  ...
  const canManageTemplates = Boolean(managerTemplateScope?.canManageTemplates);
  ...
  function useSelectedRound() {
    onMessage(`${round} manager templates selected. Continue in Source Data to generate with this round.`);
  ...
  return <div className="templates-progressive-workspace">
    <section className="panel template-manager-policy-banner" aria-label="Template manager policy">
      <p className="eyebrow">{managerTemplateAuthorityLabel(managerTemplateScope)}</p>
      <h2>{canManageTemplates ? 'Manager controls default templates' : 'Client uses assigned-manager templates'}</h2>
      <p>{managerTemplateActionLabel(managerTemplateScope)}</p>
    </section>
    ...
    {stage === 'EDITOR' && packet && <section className="template-selected-editor" aria-label={`${packetTitle(packet)} configuration`}>
      <header className="panel template-selected-command template-merged-command"><div className="template-selected-identity"><p className="eyebrow">{round} · {packetType(packet)}</p><h2>{packetTitle(packet)}</h2><p className="template-selected-order">{packetOrderText(packet)}</p><div className="template-selected-badges"><span>{positionCount(packet)}</span><span>{canManageTemplates ? 'Manager editable' : 'Read-only client view'}</span>{packet === 'DISPUTE' && <span>Order locked</span>}</div></div><div className="template-selected-actions"><button type="button" className="primary-button" onClick={useSelectedRound}>Use this round in Source Data →</button><button type="button" className="secondary-button" onClick={() => setStage('PACKET')}>Change packet</button><button type="button" className="secondary-button" onClick={chooseDifferentRound}>Change round</button></div></header>
      <TemplatePacketConfigurator round={round} slots={slots} supportingReady={supportingReady} focusedPacket={packet} embedded canManageTemplates={canManageTemplates} managerTemplateScope={managerTemplateScope} managedExhibits={managedExhibits} onUploadLetter={onUploadLetter} onRemoveLetter={onRemoveLetter} onExhibitsChange={onExhibitsChange} onMessage={onMessage} />
    </section>}
  </div>;
}
