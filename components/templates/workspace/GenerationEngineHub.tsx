...
export default function GenerationEngineHub({ context, plan }: { context: TemplateLibraryContext; plan: GenerationEnginePlan }) {
  return <section className="template-workspace-hub" data-template-workspace-hub="engine" data-template-process="template-execution-control">
    ...
    <section className="template-workspace-two-column">
      <article className="admin-monitor-card template-workspace-status-card"><p className="eyebrow">Release decision</p><strong>{plan.releaseAction.enabled ? 'Ready' : 'Not ready'}</strong><span>{plan.releaseAction.reason}</span><a className={`template-workspace-next-action ${plan.releaseAction.enabled ? 'ready' : 'blocked'}`} href="/manager-workspace/engine"><span>{plan.releaseAction.label}</span><small>{plan.releaseAction.enabled ? 'Run release validation before assigning to clients.' : 'Resolve the exact issue shown by the engine.'}</small></a></article>
      <article className="admin-monitor-card template-workspace-status-card"><p className="eyebrow">Automation safety</p><strong>{plan.blockers.length ? 'Locked' : 'Available'}</strong><span>Automation stays locked until preview, mapping, static text, and table rules are safe.</span><div className="template-rule-list">{plan.blockers.length ? plan.blockers.map((blocker) => <span key={blocker} className="template-workspace-inline-warning">{blocker}</span>) : <span className="template-workspace-inline-ok">No release blockers detected.</span>}</div></article>
    </section>
  </section>;
}
