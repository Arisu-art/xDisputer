...
function eventCount(events: AccessAuditEvent[], pattern: RegExp) {
  return events.filter((event) => pattern.test(event.event_type)).length;
}
    ...
    <section className="admin-monitor-main native-console-main">
      <header className="admin-monitor-header native-command-hero minimal-report-hero">
        <div>
          ...
          <span>Minimal access history focused on who changed access, who was affected, and what happened.</span>
        </div>
      </header>
      ...
      {errorMessage ? <section className="admin-monitor-card"><div className="admin-monitor-empty">Could not load audit events: {errorMessage}</div></section> : <>
        <section className="minimal-report-summary" aria-label="Access audit summary">
          <article><span>Events</span><strong>{events.length}</strong><small>Recent access changes</small></article>
          <article><span>Approvals</span><strong>{approvals}</strong><small>Granted or restored</small></article>
          <article className={restrictions ? 'attention' : 'complete'}><span>Restrictions</span><strong>{restrictions}</strong><small>{restrictions ? 'Review context' : 'None recent'}</small></article>
        ...
        <section className="admin-monitor-card native-operation-card minimal-activity-card">
          <div className="admin-monitor-card-header compact-card-header">
            <div><p>Audit stream</p><h2>Recent access events</h2></div>
            <span>{events.length} events</span>
          </div>
          {visibleEvents.length ? <div className="minimal-activity-list audit-event-list" role="list">
            {visibleEvents.map((event) => <article key={event.id} className="minimal-activity-row audit-event-row" role="listitem">
              <div>
                <strong>{readableAuditEventType(event.event_type)}</strong>
                <span>{event.event_detail || 'Access event'} · {formatDate(event.created_at)}</span>
              </div>
              <small>{event.actor_email || 'System'} → {event.target_email || 'Workspace'}</small>
              <em className="admin-status-badge active">{event.target_role || event.actor_role || 'event'}</em>
            </article>)}
          </div> : <div className="admin-monitor-empty">No access audit events are available yet.</div>}
        </section>
      </>}
    </section>
  </main>;
