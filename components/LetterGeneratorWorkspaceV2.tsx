...
export default function LetterGeneratorWorkspaceV2({ accountEmail, accountRole = 'client' }: { accountEmail?: string | null; accountRole?: 'admin' | 'client' }) {
  ...
  async function persistGenerationRun(manifest: GenerationManifest) { try { const response = await fetch('/api/generation-runs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clientName: manifest.clientName, round: manifest.round, manifest, status: 'generated' }) }); const payload = await response.json().catch(() => null); if (payload?.entitlement) window.dispatchEvent(new CustomEvent('xdisputer:output-entitlement-updated', { detail: payload.entitlement })); if (!response.ok) console.warn('Generation run was not saved.', payload || await response.text()); } catch (error) { console.warn('Generation run persistence failed.', error); } }
