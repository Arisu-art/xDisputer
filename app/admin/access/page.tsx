...
function viewTitle(view: string) { if (view === 'pending') return 'Pending approval'; if (view === 'active') return 'Active clients'; if (view === 'blocked') return 'Disabled / suspended'; return 'Workspace client directory'; }
function viewDescription(view: string) { if (view === 'pending') return 'Review users waiting for manager approval. Master-set client limits remain enforced by Supabase.'; if (view === 'active') return 'Review active clients and their daily output usage under the master-set agreement limit.'; if (view === 'blocked') return 'Review disabled or suspended client accounts without exposing unnecessary account data.'; return 'Approve clients, monitor capacity, and review daily output usage from one workspace control page.'; }
...
function ClientList({ accounts, entitlements }: { accounts: ManagedAccount[]; entitlements: EntitlementLimitMap }) { if (!accounts.length) return <div className="admin-monitor-empty">No workspace clients match this dataset.</div>; return <div className="account-control-list">{accounts.map((item) => <ClientFlyout key={item.id} account={item} entitlement={entitlements[item.id]} />)}</div>; }
