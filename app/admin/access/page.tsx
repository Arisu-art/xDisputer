...
export default async function AdminAccessPage({ searchParams }: PageProps) {
  ...
  return <main className="admin-monitor-page native-console manager-ops-console"><aside className="admin-monitor-sidebar native-console-sidebar"><div className="admin-monitor-brand"><span>xD</span><div><strong>xDisputer</strong><small>Manager console</small></div></div><div className="admin-sidebar-section-title">Operations</div><nav aria-label="Manager navigation"><ConsoleNavLink href="/admin">Monitoring</ConsoleNavLink><ConsoleNavLink className="active" href="/admin/access">Access control</ConsoleNavLink><ConsoleNavLink href="/admin?panel=intake">Client intake</ConsoleNavLink><ConsoleNavLink href="/admin?panel=review">Review queue</ConsoleNavLink><ConsoleNavLink href="/admin/reports">Reports</ConsoleNavLink><ConsoleNavLink href="/admin/audit">Audit log</ConsoleNavLink></nav><div className="admin-monitor-account"><strong>{profile?.email || user.email || 'Manager account'}</strong><small>Manager account</small><form action="/auth/sign-out" method="post"><button type="submit">Sign out</button></form></div></aside><section className="admin-monitor-main native-console-main"><header className="admin-monitor-header native-command-hero manager-compact-hero"><div><p>Access control</p><h1>Workspace-scoped client workflow.</h1><span>Client limits are controlled by the master account. Managers can see capacity and daily output usage but cannot edit agreement limits.</span></div></header>{(summaryError || directory.errorMessage || entitlementResult.errorMessage) && <section className="admin-monitor-card"><div className="admin-monitor-empty">{summaryError || directory.errorMessage || entitlementResult.errorMessage}</div></section>}<section className="minimal-report-summary manager-entitlement-summary" aria-label="Manager entitlement summary"><article><span>Client seats</span><strong>{managerEntitlement?.current_clients || summary.active}/{formatLimit(managerEntitlement?.max_clients)}</strong><small>Master agreement</small></article><article><span>Default outputs</span><strong>{formatLimit(managerEntitlement?.default_client_output_limit)}</strong><small>Per client per day</small></article><article><span>Active clients</span><strong>{summary.active}</strong><small>Approved accounts</small></article><article><span>Pending</span><strong>{summary.pending}</strong><small>Waiting approval</small></article></section>{selectedView === 'overview' ? <section className="progressive-dataset-grid access-workflow-grid"><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=pending"><p>Access control</p><h2>Pending approval</h2><span>{summary.pending} pending</span><strong>Review users waiting for manager approval.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=active"><p>Client usage</p><h2>Active clients</h2><span>{summary.active} active</span><strong>Review daily output usage under the master-set limit.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=blocked"><p>Access control</p><h2>Disabled / suspended</h2><span>{summary.blocked} blocked</span><strong>Review blocked client accounts.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/audit"><p>Audit</p><h2>Access history</h2><span>Events</span><strong>Review approval and account-control history.</strong></ConsoleNavLink></section> : <section className="admin-dataset-stack"><div className="access-workflow-toolbar"><ConsoleNavLink className="access-workflow-back" href="/admin/access">← Access control</ConsoleNavLink></div><section className="admin-monitor-card native-operation-card"><div className="admin-monitor-card-header"><div><p>Workspace dataset</p><h2>{viewTitle(selectedView)}</h2></div></div><DirectoryFilter view={selectedView} query={directoryParams.query} /><ClientTable accounts={directory.accounts} entitlements={entitlementResult.entitlements} /><Pager view={selectedView} query={directoryParams.query} page={directory.page} pageSize={directory.pageSize} total={directory.total} /></section></section>}</section></main>;
}
