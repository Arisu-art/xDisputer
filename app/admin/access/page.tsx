...
function viewDescription(view: string) {
  if (view === 'pending') return 'Review users waiting for manager approval. Master-set client limits remain enforced by Supabase.';
  if (view === 'active') return 'Review active clients and their daily output usage under the master-set agreement limit.';
  if (view === 'blocked') return 'Review disabled or suspended client accounts without exposing unnecessary account data.';
  return 'Approve clients, monitor capacity, and review daily output usage from one workspace control page.';
}
...
function ClientList({ accounts, entitlements }: { accounts: ManagedAccount[]; entitlements: EntitlementLimitMap }) {
  if (!accounts.length) return <div className="admin-monitor-empty">No workspace clients match this dataset.</div>;
  return <div className="account-control-list">{accounts.map((item) => <ClientFlyout key={item.id} account={item} entitlement={entitlements[item.id]} />)}</div>;
}
...
export default async function AdminAccessPage({ searchParams }: PageProps) {
  ...
  const headerTitle = selectedView === 'overview' ? 'Workspace-scoped client workflow.' : `${viewTitle(selectedView)}.`;
  const headerDescription = selectedView === 'overview' ? 'Client limits are controlled by the master account. Managers can see capacity and daily output usage but cannot edit agreement limits.' : viewDescription(selectedView);

  return <main className="admin-monitor-page native-console manager-ops-console"><aside className="admin-monitor-sidebar native-console-sidebar"><div className="admin-monitor-brand"><span>xD</span><div><strong>xDisputer</strong><small>Manager console</small></div></div><div className="admin-sidebar-section-title">Operations</div><nav aria-label="Manager navigation"><ConsoleNavLink href="/admin">Monitoring</ConsoleNavLink><ConsoleNavLink className="active" href="/admin/access">Access control</ConsoleNavLink><ConsoleNavLink href="/admin?panel=intake">Client intake</ConsoleNavLink><ConsoleNavLink href="/admin?panel=review">Review queue</ConsoleNavLink><ConsoleNavLink href="/admin/reports">Reports</ConsoleNavLink><ConsoleNavLink href="/admin/audit">Audit log</ConsoleNavLink></nav><ManagerWorkspaceSwitch /><div className="admin-monitor-account"><strong>{profile?.email || user.email || 'Manager account'}</strong><small>Manager account</small><form action="/auth/sign-out" method="post"><button type="submit">Sign out</button></form></div></aside><section className="admin-monitor-main native-console-main"><header className="admin-monitor-header native-command-hero manager-compact-hero directory-hero-with-action"><div><p>Access control</p><h1>{headerTitle}</h1><span>{headerDescription}</span></div>{selectedView !== 'overview' && <ConsoleNavLink className="directory-header-action" href="/admin/access">Access control</ConsoleNavLink>}</header>{(summaryError || directory.errorMessage || entitlementResult.errorMessage) && <section className="admin-monitor-card"><div className="admin-monitor-empty">{summaryError || directory.errorMessage || entitlementResult.errorMessage}</div></section>}<section className="minimal-report-summary manager-entitlement-summary" aria-label="Manager entitlement summary"><article><span>Client seats</span><strong>{managerEntitlement?.current_clients || summary.active}/{formatLimit(managerEntitlement?.max_clients)}</strong><small>Master agreement</small></article><article><span>Default outputs</span><strong>{formatLimit(managerEntitlement?.default_client_output_limit)}</strong><small>Per client per day</small></article><article><span>Active clients</span><strong>{summary.active}</strong><small>Approved accounts</small></article><article><span>Pending</span><strong>{summary.pending}</strong><small>Waiting approval</small></article></section>{selectedView === 'overview' ? <section className="progressive-dataset-grid access-workflow-grid"><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=pending"><p>Access control</p><h2>Pending approval</h2><span>{summary.pending} pending</span><strong>Review users waiting for manager approval.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=active"><p>Client usage</p><h2>Active clients</h2><span>{summary.active} active</span><strong>Review daily output usage under the master-set limit.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/access?view=blocked"><p>Access control</p><h2>Disabled / suspended</h2><span>{summary.blocked} blocked</span><strong>Review blocked client accounts.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/admin/audit"><p>Audit</p><h2>Access history</h2><span>Events</span><strong>Review approval and account-control history.</strong></ConsoleNavLink></section> : <section className="admin-dataset-stack single-header-dataset"><section className="admin-monitor-card native-operation-card"><DirectoryFilter view={selectedView} query={directoryParams.query} /><ClientList accounts={directory.accounts} entitlements={entitlementResult.entitlements} /><Pager view={selectedView} query={directoryParams.query} page={directory.page} pageSize={directory.pageSize} total={directory.total} /></section></section>}</section></main>;
}
