...
import { managerOperationsNavItems, normalizeManagerOperationsPanel, type ManagerOperationsPanel } from '../../lib/manager-console/manager-operations-panels';
...
export default async function AdminPage({ searchParams }: PageProps) {
  ...
  const activeDefinition = managerOperationsNavItems(activePanel).find((item) => item.active);
  ...
  return <ManagerConsoleShell
    ...
    header={{ eyebrow: 'Manager console', title: activeDefinition?.label || 'Monitoring', description: activeDefinition?.label === 'Payroll' ? 'Compute pay from user rate, salary, and output count.' : activeDefinition?.label === 'Access control of user' ? 'Manage user account status and operational metadata.' : activeDefinition?.label === 'Request' ? 'Review pending confirmations and invite requests.' : activeDefinition?.label === 'Report' ? 'Generate a clean operational report.' : 'Monitor outputs and user status from one clean panel.' }}
  >
