...
function back(request: NextRequest, state: string, message: string) {
  const target = new URL(request.headers.get('referer') || '/admin/output-activity', request.url);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const status = action === 'confirm' ? 'approved' : action === 'reject' ? 'rejected' : action === 'paid' ? 'paid' : '';
    ...
    await createNotification({ supabase, createdBy: user.id, recipientUserId: existing.data.disputer_id, title: status === 'approved' ? 'Output confirmed' : status === 'rejected' ? 'Output not accepted' : 'Output paid', body: existing.data.output_label, href: '/workspace', severity: status === 'rejected' ? 'error' : 'success' });
    revalidatePath('/admin/output-activity');
