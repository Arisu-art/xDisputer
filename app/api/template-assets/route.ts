...
async function deleteAssetRecordsAndFiles(session: SessionContext, managerUserId: string, assets: Array<{ id: string; storage_bucket: string; storage_path: string }>) { if (!assets.length) return { deleted: 0, warning: null as string | null }; const bucketGroups = new Map<string, string[]>(); assets.forEach((asset) => { const bucket = asset.storage_bucket || 'template-assets'; const paths = bucketGroups.get(bucket) || []; paths.push(asset.storage_path); bucketGroups.set(bucket, paths); }); for (const [bucket, paths] of Array.from(bucketGroups.entries())) { const storageDelete = await removeManagerTemplateObjects({ sessionSupabase: session.supabase, bucket, paths }); if (storageDelete.error) return { deleted: 0, warning: storageDelete.error.message }; } const ids = assets.map((asset) => asset.id); const tableDelete = await session.supabase.from('template_assets').delete().eq('manager_user_id', managerUserId).in('id', ids); if (tableDelete.error) return { deleted: 0, warning: tableDelete.error.message }; return { deleted: assets.length, warning: null }; }
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ assets: autoBackfill.assets, managerTemplateScope: managerTemplateScopePayload(scope), templateStorage: { mode: managerTemplateStorageMode() }, dynamicTemplateEngineV2: { rendererMode, autoBackfilled: autoBackfill.backfilledCount, warnings: autoBackfill.warnings } });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (insert.error) { await removeManagerTemplateObjects({ sessionSupabase: session.supabase, bucket: 'template-assets', paths: [storagePath] }); return respond(request, 'error', insert.error.message, 500); }
    ...
    if (activation.warning) { await removeManagerTemplateObjects({ sessionSupabase: session.supabase, bucket: 'template-assets', paths: [storagePath] }); await session.supabase.from('template_assets').delete().eq('manager_user_id', scope.managerUserId).eq('id', insert.data.id); return respond(request, 'error', activation.warning, 500); }
    uploadedPath = null; insertedAssetId = null;
    return respond(request, 'ok', `${round} ${targetType} manager template saved as active version. ${activation.archived} previous active version(s) archived.`, 200, { assetId: insert.data.id, archivedVersions: activation.archived, activationMode: activation.mode, contentHash, managerTemplateScope: managerTemplateScopePayload(scope), templateStorage: { mode: managerTemplateStorageMode() }, validation: contract.validation, dynamicTemplateEngineV2: { rendererMode, diagnosticsWarning: dynamicV2.warning, contract: dynamicContractSummary(dynamicV2.contract) } });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return respond(request, 'ok', `${round} ${resolvedLetterType || resolvedExhibitKind} manager template removed from Supabase.`, 200, { deleted: cleanup.deleted, managerTemplateScope: managerTemplateScopePayload(scope), templateStorage: { mode: managerTemplateStorageMode() } });
  } catch (error) {
    if (error instanceof ManagerTemplateScopeError) return managerScopeFailure(request, error);
