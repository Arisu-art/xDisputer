...
export default async function MasterAccountsPage({ searchParams }: PageProps) {
  ...
  return <main className="admin-monitor-page native-console master-ops-console"><aside className="admin-monitor-sidebar native-console-sidebar"><div className="admin-monitor-brand"><span>xD</span><div><strong>xDisputer</strong><small>Workspace accounts</small></div></div><div className="admin-sidebar-section-title">Operations</div><nav aria-label="Master navigation"><ConsoleNavLink href="/master">Monitoring</ConsoleNavLink><ConsoleNavLink className="active" href="/master/accounts">All accounts</ConsoleNavLink><ConsoleNavLink href="/master/workspaces">Workspaces</ConsoleNavLink><ConsoleNavLink href="/master/reports">Reports</ConsoleNavLink><ConsoleNavLink href="/master/audit">Audit log</ConsoleNavLink><ConsoleNavLink href="/master/system">System health</ConsoleNavLink></nav><div className="admin-monitor-account"><strong>{profile?.email || user.email || 'Master account'}</strong><small>Owner account</small><form action="/auth/sign-out" method="post"><button type="submit">Sign out</button></form></div></aside><section className="admin-monitor-main native-console-main"><header className="admin-monitor-header native-command-hero master-compact-hero directory-hero-with-action"><div><p>Master account directory</p><h1>Workspace-scoped account workflow.</h1><span>Edit manager client-seat limits and daily client output limits from the master account. Enforcement happens in Supabase and the generation API.</span></div>{selectedView !== 'overview' && <ConsoleNavLink className="directory-header-action" href="/master/accounts">Account directory</ConsoleNavLink>}</header>{(summaryError || directory.errorMessage || entitlementResult.errorMessage) && <section className="admin-monitor-card"><div className="admin-monitor-empty">{summaryError || directory.errorMessage || entitlementResult.errorMessage}</div></section>}{selectedView === 'overview' ? <section className="progressive-dataset-grid access-workflow-grid"><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/master/accounts?view=managers"><p>Manager limits</p><h2>Workspace managers</h2><span>{summary.managers} manager(s)</span><strong>Set client-seat limits and default daily output limits.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/master/accounts?view=clients"><p>Client limits</p><h2>Workspace clients</h2><span>{summary.clients} client(s)</span><strong>Set per-client daily output caps and review usage.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/master/accounts?view=pending"><p>Pending</p><h2>Pending / unassigned</h2><span>{summary.pending} pending</span><strong>Find users who need manager assignment or approval.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/master/accounts?view=blocked"><p>Blocked</p><h2>Disabled / suspended</h2><span>{summary.blocked} blocked</span><strong>Review accounts that cannot use the platform.</strong></ConsoleNavLink><ConsoleNavLink className="progressive-dataset-card access-workflow-card" href="/master/workspaces"><p>Workspace</p><h2>Tenant directory</h2><span>{coverageRate}% coverage</span><strong>Open workspace membership and assignment visibility.</strong></ConsoleNavLink></section> : <section className="master-access-stack"><div className="access-workflow-toolbar"><span>{directory.total} result(s)</span></div><article className="admin-monitor-card native-operation-card"><div className="admin-monitor-card-header"><div><p>Workspace dataset</p><h2>{viewTitle(selectedView)}</h2></div><span>{directory.total} total</span></div><DirectoryFilter view={selectedView} query={directoryParams.query} /><MasterAccountTable accounts={directory.accounts} currentUserId={user.id} emptyText="No accounts match this workspace dataset." entitlements={entitlementResult.entitlements} /><Pager view={selectedView} query={directoryParams.query} page={directory.page} pageSize={directory.pageSize} total={directory.total} /></article></section>}</section></main>;
}
