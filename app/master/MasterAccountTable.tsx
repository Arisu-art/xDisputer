...
function LimitEditor({ account, entitlement }: { account: ManagedAccount; entitlement?: EntitlementLimitRow }) {
  ...
  return <form action="/api/master/entitlements" method="post" className="limit-editor-form compact-limit-editor">
    ...
    <input type="hidden" name="profileId" value={account.id} />
    <div className="limit-meter"><strong>{entitlement?.output_used_this_month || 0}/{formatLimit(entitlement?.effective_output_limit)}</strong><small>outputs</small></div>
  ...
  return (
    <div className="admin-monitor-table-wrap">
      <table className="admin-monitor-table professional-data-table entitlement-table">
        <thead><tr><th>Account</th><th>Role</th><th>Status</th><th>Link</th><th>Limits</th><th>Invite</th><th>Updated</th><th>Controls</th></tr></thead>
        <tbody>
          {accounts.length ? accounts.map((item) => (
            <tr key={item.id}>
              <td data-label="Account"><strong>{item.full_name || item.email || 'Unnamed user'}</strong><small>{item.email || 'Protected account'}</small></td>
              <td data-label="Role"><span className={`admin-role-badge ${item.role}`}>{item.role === 'admin' ? 'manager' : item.role}</span></td>
              <td data-label="Status"><span className={`admin-status-badge ${item.account_status || 'pending'}`}>{statusText(item.account_status)}</span></td>
          ...
          )) : <tr><td colSpan={8} className="admin-monitor-empty">{emptyText}</td></tr>}
        </tbody>
      </table>
    </div>
  );
