...
function Flyout({ account, currentUserId, limit }: { account: ManagedAccount; currentUserId: string; limit?: EntitlementLimitRow }) {
  return <details className="table-flyout account-action-flyout"><summary><span>{agreementSummary(account, limit)}</span><small>Manage</small></summary><div className="table-flyout-card"><header><div><p>Account controls</p><h3>{account.full_name || account.email || 'Account'}</h3></div></header><section><strong>Daily agreement limits</strong><LimitForm account={account} limit={limit} /></section><section><strong>Actions</strong><ActionForms account={account} currentUserId={currentUserId} /></section></div></details>;
}
